...
public final class TwemojiFontInjection {
    ...
    private static CompletableFuture<List<GlyphProvider>> pending = CompletableFuture.completedFuture(List.of());
    private static List<GlyphProvider> injected = List.of();
    ...
    public static void prepare(ResourceManager resourceManager, Executor executor) {
        CompletableFuture<List<GlyphProvider>> loaded = CompletableFuture.supplyAsync(() -> loadProviders(resourceManager), executor)
            .exceptionally(error -> {
                Twemoji.LOGGER.warn("Failed to prepare injected Twemoji font providers", error);
                return List.of();
            });
        synchronized (LOCK) {
            for (GlyphProvider provider : injected) {
                superseded.add(provider);
            }
            injected = List.of();
            pending = loaded;
        }
    ...
    public static List<GlyphProvider> awaitProvidersToInject() {
        CompletableFuture<List<GlyphProvider>> future;
        synchronized (LOCK) {
            future = pending;
        }
        List<GlyphProvider> loaded;
        try {
            loaded = future.join();
        } catch (Exception e) {
