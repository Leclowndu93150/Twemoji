...
public final class EmojiPicker {
    ...
    public static boolean handleActiveSearchChar(EditBox input, char c) {
        return activeSearchPicker != null && activeSearchPicker.searchOwner == input && activeSearchPicker.charTyped(c);
    }
    ...
    public boolean mouseClicked(double mx, double my, int button, int screenWidth, int screenHeight, EditBox input) {
        int mouseX = (int) mx;
        int mouseY = (int) my;
        if (EmojiConfig.get().isPickerButtonVisible()
            && contains(mouseX, mouseY, buttonX(screenWidth) - 1, buttonY(screenHeight), BUTTON_SIZE + 2, BUTTON_SIZE)) {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers, EditBox input) {
        if (!this.open) return false;
        if (keyCode == 256) {
            this.close();
            return true;
    ...
    public boolean charTyped(char c) {
        if (!this.open || !this.searchFocused || !SharedConstants.isAllowedChatCharacter(c)) return false;
        this.search += c;
        this.scroll = 0;
        this.cursorIndex = -1;
        return true;
    ...
    private void renderFooter(GuiGraphics graphics, int x, int y) {
        ...
        if (entry == null) {
            List<EmojiRegistry.EmojiEntry> entries = this.entries();
            if (!entries.isEmpty()) entry = entries.get(0);
        }
    ...
    private String selectedLabel() {
        if (this.selected instanceof BuiltinCategory b) return b.category().label();
        if (this.selected instanceof CustomCategory c) return c.name();
        throw new IllegalStateException("Unknown category: " + this.selected);
    }
    ...
    private List<EmojiRegistry.EmojiEntry> entriesFor(PickerCategory cat) {
        if (cat instanceof BuiltinCategory b) return entriesForBuiltin(b.category());
        if (cat instanceof CustomCategory c) return List.copyOf(EmojiRegistry.INSTANCE.getCustomCategoryEntries().getOrDefault(c.name(), List.of()));
        throw new IllegalStateException("Unknown category: " + cat);
    }
    ...
    private void renderPickerCategoryIcon(GuiGraphics graphics, PickerCategory cat, int x, int y) {
        if (cat instanceof BuiltinCategory b) {
            renderCategoryIcon(graphics, b.category(), x, y);
        } else if (cat instanceof CustomCategory c) {
            ...
            if (iconEntry == null) {
                ...
                iconEntry = entries.isEmpty() ? null : entries.get(0);
            }
            if (iconEntry != null) {
