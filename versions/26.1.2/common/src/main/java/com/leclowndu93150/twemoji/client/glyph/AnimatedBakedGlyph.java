...
public final class AnimatedBakedGlyph implements BakedGlyph {
    ...
    public TextRenderable.@Nullable Styled createGlyph(float x, float y, int color, int shadowColor, Style style, float boldOffset, float shadowOffset) {
        return new Instance(emoji, x, y, style);
    }

    private static final class Instance implements TextRenderable.Styled {
        private final AnimatedEmojiRegistry.AnimatedEmoji emoji;
        private final float x;
        private final float y;
        private final Style style;
        private @Nullable Identifier cachedFrame;

        Instance(AnimatedEmojiRegistry.AnimatedEmoji emoji, float x, float y, Style style) {
            this.emoji = emoji;
            this.x = x;
            this.y = y;
            this.style = style;
        ...
        private Identifier frame() {
            if (cachedFrame == null) cachedFrame = emoji.currentFrame();
            return cachedFrame;
        }
        ...
        public RenderType renderType(Font.DisplayMode displayMode) {
            ...
            return switch (displayMode) {
                case NORMAL -> RenderTypes.text(id);
                case SEE_THROUGH -> RenderTypes.textSeeThrough(id);
                case POLYGON_OFFSET -> RenderTypes.textPolygonOffset(id);
            };
        }

        @Override
        public GpuTextureView textureView() {
            AbstractTexture texture = Minecraft.getInstance().getTextureManager().getTexture(frame());
