...
public final class AnimatedLogo {
    ...
    private static final List<Identifier> FRAME_IDS = new ArrayList<>();
    ...
    private static void decode(InputStream stream) throws Exception {
        ...
        try (ImageInputStream input = ImageIO.createImageInputStream(stream)) {
            ...
            for (int i = 0; i < frameCount; i++) {
                ...
                int frameIndex = i;
                DynamicTexture texture = new DynamicTexture(() -> "twemoji_logo_frame_" + frameIndex, image);
                Identifier id = Identifier.fromNamespaceAndPath(Twemoji.MOD_ID, "logo/frame_" + i);
    ...
    private static NativeImage bufferedToNative(BufferedImage img) {
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                out.setPixel(x, y, pixels[y * w + x]);
            }
        }
        return out;
    }

    public static Identifier currentFrameId() {
        ensureLoaded();
