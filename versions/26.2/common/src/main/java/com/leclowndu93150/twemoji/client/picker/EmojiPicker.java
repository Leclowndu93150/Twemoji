...
public final class EmojiPicker {
    ...
    private void renderNav(GuiGraphicsExtractor graphics, int mouseX, int mouseY, int x, int y) {
        ...
        graphics.text(this.font, renderedText, searchX + 5, searchY + 4, color, false);
        ...
        graphics.text(this.font, "o", searchX + searchWidth(x) - 11, searchY + 4, MUTED_COLOR, false);
        ...
        if (gearHovered) graphics.requestCursor(CursorTypes.POINTING_HAND);
    ...
    private void openConfigScreen() {
        ...
        this.close();
        client.gui.setScreen(TwemojiConfigScreen.create(parent));
    }
    ...
    private void renderGrid(GuiGraphicsExtractor graphics, int mouseX, int mouseY, int x, int y) {
        List<EmojiRegistry.EmojiEntry> entries = this.entries();
        int gridX = gridX(x);
        int gridY = gridY(y);
    ...
    private void renderFooter(GuiGraphicsExtractor graphics, int x, int y) {
        ...
        graphics.text(this.font, trimToWidth(entry.shortcode(), PANEL_WIDTH - CATEGORY_WIDTH - 29), x + CATEGORY_WIDTH + 27, fy + 8, TEXT_COLOR, false);
    }
    ...
    private void renderToneMenu(GuiGraphicsExtractor graphics, int mouseX, int mouseY, int x, int y) {
    ...
    private void renderCategoryScrollbar(GuiGraphicsExtractor graphics, int x, int y, int total) {
    ...
    private void renderScrollbar(GuiGraphicsExtractor graphics, int x, int y, int height, int total, int visible, int offset, int trackWidth) {
    ...
    private void renderToneEmoji(GuiGraphicsExtractor graphics, int tone, int x, int y, int size) {
    ...
    private void renderCategoryIcon(GuiGraphicsExtractor graphics, Category category, int x, int y) {
    ...
    private static void renderFace(GuiGraphicsExtractor graphics, int x, int y, int size, int fill, int ink) {
